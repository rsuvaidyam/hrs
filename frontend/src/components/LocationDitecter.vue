<template>
    <div v-if="!pinCodes.includes(store.location_ditecter) || store.openpop"
        class="w-full h-screen p-16 bg-black bg-opacity-40 fixed z-50">
        <div class="bg-white rounded-lg shadow-lg p-6 max-w-xl w-full text-center animate-vibrate">
            ...
            <div class="mt-6 flex items-center justify-center gap-4">
                <Button @click="detectLocation" label="Detect my location" :variant="'outline'" theme="blue" size="lg"
...
function setPinCode() {
    if (selectedPinCode.value) {
        ...
        // location_ditecter.value = selectedPinCode.value;
        store.location_ditecter =  selectedPinCode.value
        store.openpop =  false
    }
}selectedPinCode

// Watcher to update location_ditecter whenever localStorage changes

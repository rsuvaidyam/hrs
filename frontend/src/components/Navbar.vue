  ...
  <div class="w-full fixed top-0 h-12 md:h-14 z-40 bg-[#982B1C] shadow-sm">
    <div class="px-3 md:px-8 flex justify-between items-center h-full max-w-[1800px] mx-auto w-full">
      ...
      <div class="text-white cursor-pointer">
        <p class="text-xl font-bold mb-1">Delivery in 10 minutes</p>
        <div class="flex items-center" @click="store.openpop = true">
          <p v-if="store.location_ditecter" class="text-sm">New Delhi, Delhi 841311, India</p>
      ...
      <div class="flex items-center gap-3 md:gap-6">
        ...
        <router-link v-if="route.path !== '/cart'" to="/cart" class="order-1 md:order-2 text-white relative">
          ...
          <span v-if="count >= 1"
            class="w-4 h-4 rounded-full bg-white border border-gray-900 absolute -top-1 -right-2 flex items-center justify-center bg-primary text-xs font-bold text-gray-700">{{ count }}</span>
        </router-link>
      </div>
    </div>
  </div>
